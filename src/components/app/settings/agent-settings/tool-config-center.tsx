...
import { TOOL_CATEGORIES } from "@/lib/agents/tool-registry";
...
interface ToolConfigCenterProps {
  panelId: string;
  title: string;
  description: string;
...
export function ToolConfigCenter({
  panelId,
  title,
  description,
  availableTools,
  ...
  onToolConfigChange,
  onToolConfigsChange,
}: ToolConfigCenterProps) {
  const coreToolIdSet = useMemo(() => new Set(coreToolIds), [coreToolIds]);
  const [keyword, setKeyword] = useState("");
  const effectiveEnabledTools = useMemo(() => {
    const enabled = new Set(defaultEnabledToolIds);
    ...
    return [...enabled];
  }, [defaultEnabledToolIds, toolConfigs]);
    ...
    <Card className="border-border/70">
      ...
      <CardContent className="space-y-5 px-4 pb-4 sm:px-5 sm:pb-5">
        <div className="space-y-2">
          <Label htmlFor={`tool-search-${panelId}`} className="text-xs text-muted-foreground">检索工具</Label>
          <div className="relative">
            <SearchIcon className="pointer-events-none absolute top-1/2 left-2.5 size-4 -translate-y-1/2 text-muted-foreground" />
            <Input
              id={`tool-search-${panelId}`}
              value={keyword}
          ...
          <section key={category} className="space-y-3">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {toolsByCategory[category].map((tool) => {
                ...
                const isCore = coreToolIdSet.has(tool.id);
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1 space-y-1">
                      <span className="flex items-center gap-2">
                        <Label htmlFor={`tool-${panelId}-${tool.id}`} className="truncate text-sm font-medium">{tool.name}</Label>
                        {isCore && (
