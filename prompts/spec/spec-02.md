...
- 阅读src\app\api\chat\route.ts文件，我需要你使用渐进式披露逐步给大模型信息，刚开始应该直接给大模型所有的仓库名、topic、描述，大模型根据这些信息判断该推荐哪个，这样可以避免遗漏。这些信息直接在初始提示词就提供给大模型。
- 然后大模型能基于这些基础信息知道该怎么推荐。

# 用户需求

- 用户想要让 AI 获取自己的 Star 仓库，然后通过问答的方式从众多的仓库当中找到自己想要的那个。
- 可以通过一步步引导的方式来慢慢找到结果。希望能够读取所有的仓库来获得更好的答案。
